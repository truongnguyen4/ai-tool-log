...
/**
 * Settings key of a table's header layout. A table whose columns were
 * redesigned gets a new key, so a layout saved for the old columns is never
 * restored onto the new ones.
 */
QString headerKey(const QTableView *table)
{
    if (table->objectName() == QLatin1String("tablePropertyDefinitions"))
        return table->objectName() + QLatin1String("/header.v2");
    return table->objectName() + QLatin1String(kHeaderSuffix);
}
} // namespace
...
void UiManager::saveLayoutPreferences()
{
    ...
    for (QTableView *table : persistedTables()) {
        if (table && table->horizontalHeader())
            settings.setValue(headerKey(table), table->horizontalHeader()->saveState());
    }
...
void UiManager::restoreLayoutPreferences()
{
    ...
    for (QTableView *table : persistedTables()) {
        if (!table || !table->horizontalHeader())
            continue;
        const QString key = headerKey(table);
...
void UiManager::onAppSettingsClicked()
{
    ...
    SettingsDialog dlg(m_viewFont, defaultViewFont(), colVis, m_mainWindow);
    if (dlg.exec() == QDialog::Accepted) {
        // Applying a font rebuilds the application stylesheet, so skip it when
        // the family and size are what they already were.
        const std::optional<QFont> viewFont = dlg.viewFont();
        const bool fontChanged =
            viewFont.has_value() != m_viewFont.has_value()
            || (viewFont && (viewFont->family() != m_viewFont->family()
                             || viewFont->pointSize() != m_viewFont->pointSize()));
        if (fontChanged)
            applyViewFont(viewFont);
