...
void UiManager::onFileLoadFinished()
{
    ...
    // Clear the time range: it belongs to the old file's timeline and would
    // only hide lines of the new one. The filter, highlight and level controls
    // are deliberately left alone — they are the user's current search, not a
    // property of the old file.
    for (QLineEdit *filter : {m_ui->txtStartTime, m_ui->txtEndTime}) {
        const QSignalBlocker blocker(filter);
        filter->clear();
    }

    // Run the filter rather than assuming everything passes: a filter query or
    // a level floor may still be set, and the view must agree with the controls.
    applyFilters();
...
void UiManager::onSaveMarkedClicked()
{
    const QVector<LogEntry> entries = activeMarkLogModel()->markedEntries();
    if (entries.isEmpty()) {
        flashStatus(tr("Nothing is marked yet — double-click a log row to mark it"));
        return;
    }

    const QString path = QFileDialog::getSaveFileName(
        m_mainWindow, tr("Save Marked Lines"),
        QDir::homePath() + QStringLiteral("/marked-logs.txt"),
        tr("Text (*.txt);;Log (*.log);;All files (*)"));
    if (path.isEmpty())
        return;

    QString errorMsg;
    if (m_fileManager.saveToFile(path, entries, errorMsg))
        flashStatus(tr("Saved %1 marked line(s) to %2").arg(entries.size()).arg(path));
    else
        flashStatus(tr("Failed to save: %1").arg(errorMsg));
}
