...
private:
    ...
    /** Toolbar toggles for the file row and the marked-lines panel. */
    void setupLogViewToggles();
    void setFileRowVisible(bool visible);
    void setMarkedPanelVisible(bool visible);
    /** Keep the marked-panel and crash buttons in step with the marked lines. */
    void updateMarkedPanel();
    /** Select the next marked crash or ANR in the active pane. */
    void onNextCrashClicked();
    /** Save the marked lines to a file: the start of a bug report. */
    void onSaveMarkedClicked();
    /** Drop down the device apps, those seen in this log first, with a search box. */
    void showAppFilterPopup();
    /** Give lines that arrived before their process was known their app name. */
    void fillMissingPackages();
    ...
    /** The line under the filter box: unapplied edit, parse warning or syntax hint. */
    void updateQueryHint();
    void onSettingsFilterChanged();
    void onPropertiesFilterChanged();
    /** Row count and repair warning beside the Settings / Properties filter. */
    void updateSettingsFilterStatus();
    void updatePropertiesFilterStatus();
    void showTableFilterStatus(class QLineEdit *edit, class QLabel *status,
                               const TableQuery &query, int visibleRows, int totalRows);
    ...
    //
    // The device selector lists every device seen, online or not, so one that
    // is rebooting can still be chosen — Logcat then waits for it.
    ...
    /** Load the remembered devices and the last chosen one. */
    void loadKnownDevices();
    void saveKnownDevices() const;
    /** Remember @p devices, all online, keeping the list bounded. */
    void rememberDevices(const QList<AdbDevice> &devices);
    /** Refill the selector: online devices, the ones away, then the actions. */
    void rebuildDeviceSelector();
    /**
     * Make @p deviceId — the chosen device while it is online, else empty —
     * the one every tab talks to, reloading or clearing their data.
     */
    void setActiveDevice(const QString &deviceId);
    void addDeviceBySerial();
    void forgetOfflineDevices();
    ...
    /** Reset the active pane and install @p converter. False without @p deviceId. */
    bool beginCapture(const LogConverterPtr &converter, const QString &deviceId);
    ...
    /** The "Follow reboots" toggle beside Logcat. */
    void setupCaptureControls();
    /** Logcat button text, state and tooltip: idle, waiting or capturing. */
    void updateCaptureButtons();
    /** A marked line in the log itself: a disconnect, a reconnect, a reboot. */
    void appendCaptureNote(const QString &text, AdbManager::CaptureKind kind);
    ...
    /** Add "field:value" to the filter box, or "-field:value" when @p exclude. */
    void addQueryTerm(LogQuery::Field field, const QString &value, bool exclude);
    ...
    /**
     * Give the log tables and read-only output views @p font, or the theme
     * default when it is empty, and remember the choice for the next session.
     * Buttons, inputs and every other control keep the application font.
     */
    void applyViewFont(const std::optional<QFont> &font);
    /** The view font saved by applyViewFont(), if one was chosen. */
    static std::optional<QFont> savedViewFont();
    /** What the log tables show while no view font is chosen. */
    QFont defaultViewFont() const;
    /** Size a log table's default row height for the current view font. */
    void fitLogRowHeight(QTableView *view);
    ...
    // Devices. m_currentDeviceId is the chosen device while it is online and
    // empty otherwise — what every tab talks to. m_selectedSerial stays the
    // choice while that device is away, which is what Logcat waits for.
    struct KnownDevice {
        QString serial;
        QString model;
    };
    ...
    QVector<KnownDevice> m_knownDevices;          ///< newest first
    QSet<QString>        m_onlineSerials;
    bool                 m_deviceChoicePinned = false;   ///< picked by the user or a capture
    QPushButton         *m_btnFollowReboots   = nullptr;
    QPushButton         *m_btnFileRow         = nullptr;
    QPushButton         *m_btnMarkedPanel     = nullptr;
    QPushButton         *m_btnCrashes         = nullptr;
    QPushButton         *m_btnSaveMarked      = nullptr;
    QPushButton         *m_btnAppFilter       = nullptr;
    class SearchableListPopup *m_appFilterPopup = nullptr;
    class PackageResolver *m_packageResolver  = nullptr;
    QSet<QString>        m_pidsSeen;              ///< pids this log has shown
    int                  m_lastCrashRow       = -1;
    int                  m_lastMarkedCount    = 0;
    QElapsedTimer        m_captureWaitClock;
    ...
    // Font of the log tables and output views; empty = the theme default.
