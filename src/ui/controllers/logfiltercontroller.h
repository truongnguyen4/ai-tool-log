...
//
// The criteria built from the widgets is cached. Building it parses the
// filter query, which is cheap once per filter change but ruinous once per
// log line — and passesCurrent() is called per line. Callers refresh the
// cache with refreshCriteria() whenever the filter widgets change.
//
// The query language itself is LogQuery, in src/filters/logquery.h.
// ---------------------------------------------------------------------------
