...
struct ConfigFilterCriteria {
    ...
    QString valueFilter;
    ConfigFilterOperator valueOperator = ConfigFilterOperator::OR;
};
...
public:
    ...
    // Check if a name (and optionally value) passes the filter
    virtual bool passesFilter(const QString &name, const QString &value, const ConfigFilterCriteria &criteria) const = 0;
};
