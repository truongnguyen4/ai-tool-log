...
bool ConfigFilter::passesFilter(const QString &name, const QString &value, const ConfigFilterCriteria &criteria) const
{
    ...
    // Value filter (OR/AND logic)
    if (!criteria.valueFilter.isEmpty()) {
        if (!matchesStringFilter(value, criteria.valueFilter, criteria.valueOperator)) {
            return false;
        }
    }
