...
public:
    ...
    /**
     * Paint keyword backgrounds only as tall as the text rather than the whole
     * cell, so matches in a compact list with no gap between rows do not run
     * together into stripes. Applies to single-line cells.
     */
    void setTightHighlight(bool enabled) { m_tightHighlight = enabled; }
