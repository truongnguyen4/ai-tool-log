                                            ...
                                            const QString &text, bool selected) const
{
    ...
    while (!remaining.isEmpty() && x < textRect.right()) {
        ...
        const QRect fill = m_tightHighlight
                               ? QRect(x, baseline - metrics.ascent(), matchWidth, metrics.height())
                               : QRect(x, textRect.top(), matchWidth, textRect.height());
        painter->fillRect(fill, matchColor);
