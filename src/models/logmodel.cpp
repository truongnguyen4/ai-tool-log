...
int LogModel::fillMissingPackages(const std::function<QString(const QString &)> &lookup)
{
    int filled = 0;
    for (LogEntry &entry : m_logs) {
        if (!entry.package.isEmpty() || entry.pid.isEmpty())
            continue;
        const QString package = lookup(entry.pid);
        if (package.isEmpty())
            continue;
    ...
    if (filled > 0)
        refreshColumn(TableConfig::LogColumns::PACKAGE, {Qt::DisplayRole});
    return filled;
}
