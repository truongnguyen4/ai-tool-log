...
public:
    ...
    /**
     * Fill in packages that were unknown when their lines arrived, using
     *  lookup on each pid. Returns how many rows gained one.
     */
    int fillMissingPackages(const std::function<QString(const QString &)> &lookup);
